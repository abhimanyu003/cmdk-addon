---
title: CMDK Privacy Policy
weight: 10
---

CMDK is a command palette for your browser. It runs entirely inside your browser. We do not operate servers, collect data, or track you in any way.
...
CMDK uses two types of permissions: 
...
| **activeTab** | Lets the palette interact with the page you're currently viewing — injecting the command palette UI, capturing a screenshot when you ask for one, and running page-level commands (scroll, zoom, print). | Only the tab you're actively on, and only when you invoke CMDK. |
...
These are off by default. You enable them from the CMDK settings page, and you can revoke them at any time.

| Permission | What it does | What it accesses |
|---|---|---|
| **bookmarks** | Lets you search and open bookmarks from the palette. | Your bookmark tree (titles and URLs). Read-only — CMDK can also bookmark the current page if you use that command. |
...
| **clipboardWrite** | Lets CMDK copy things to your clipboard — URLs, screenshot images, QR codes, calculator results. | Writes to your clipboard only when you explicitly trigger a copy action. Never reads your clipboard. |
...
CMDK makes exactly **one** type of network request, and only under specific circumstances:
...
- **What is sent:** Your license key and the CMDK product ID. Nothing else — no device info, no browser fingerprint, no usage data, no personal information.
...
- **Offline behavior:** If the network request fails, CMDK continues working with the last known license state. It does not block you or retry aggressively.
...
If you have questions about how CMDK handles your data, open an issue on the repository or reach out directly. Happy to clarify anything.