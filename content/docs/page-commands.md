...
CMDK includes a set of commands that act directly on the page you're currently viewing. These are always available — no extra permissions needed beyond the required `activeTab` permission.
...
CMDK has a built-in calculator. Type a math expression directly in the palette (e.g., `12 * 8` or `(100 + 50) / 3`) and the result appears instantly. Press **`Enter`** to copy the result to your clipboard.
